...
SDL_Surface *new_Surface_PutString(SDL_Surface * parent, const char *text) {
...
void XCenteredString2(SDL_Surface * Surface, SFont_FontInfo * Font, int y,
		      char *text) {
  PutString2(Surface, &InternalFont, Surface->w / 2 - TextWidth(text) / 2, y,
	     text);
}
...
int SFont_Input2(SDL_Surface * Dest, SFont_FontInfo * Font, int x, int y,
		 int PixelWidth, char *text) {
  SDL_Event event;
  SDL_Rect rect;
  int ofs = (text[0] - 33) * 2 + 1;
  int leftshift;
  int ch = 0; //Fixes a warning: may be used unintialised.
  char *sptr;
  ...
  while(SDL_PollEvent(&event) && event.type == SDL_KEYDOWN) {
	...
	text[strlen(text) - 1] = '\0';
      ...
      for(sptr = text; *sptr; ++sptr) ; //Find end of string.
      *sptr++ = ch;
      *sptr++ = '\0';
    }
    ...
    if(ofs > 0) {
      SDL_UpdateRect(Dest,
		     x - (Font->CharPos[ofs] - Font->CharPos[ofs - 1]) / 2, y,
		     PixelWidth, Font->Surface->h);
    }
